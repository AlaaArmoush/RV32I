...
module alu_sva (
  ...
  always_comb begin : a_last_bit_flag
    assert (last_bit === alu_result[0])
      ...
      $error(
          "[alu_sva] A2 FAIL: last_bit=%b but alu_result[0]=%b (result=0x%08h)",
          last_bit,
          alu_result[0],
          alu_result
      );
  ...
  always_comb begin : a_invalid_zero
    if (alu_control == ALU_INVALID)
      assert (alu_result[0] == 32'b0)
      else
        $error(
            "[alu_sva] A3 FAIL: invalid op (0x%0h) produced non-zero result 0x%08h",
            alu_control,
            alu_result
        );
  end
  ...
  always_comb begin : a_sll_uses_shamt
    if (alu_control == ALU_SLL)
      assert (alu_result == (src1 << shamt))
        ...
        $error(
            "[alu_sva] A4 FAIL: SLL result=0x%08h, expected src1<<shamt=0x%08h (src1=0x%08h shamt=%0d)",
            alu_result,
            src1 << shamt,
            src1,
            shamt
  ...
  always_comb begin : a_sra_uses_shamt
    if (alu_control == ALU_SRA)
      assert (alu_result == logic'($signed(src1) >>> shamt))
      else
        $error(
            "[alu_sva] A4 FAIL: SRA result=0x%08h, expected src1>>>shamt=0x%08h (src1=0x%08h shamt=%0d)",
            alu_result,
            $signed(
                src1
            ) >>> shamt,
