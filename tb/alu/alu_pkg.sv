package alu_pkg;
  ...
  //relationship between src1 & src2
  typedef enum logic [1:0] {
    CMP_EQ,
    CMP_LT,
    CMP_GT
  ...
  function automatic cmp_rel_e classify_cmp(logic [31:0] a, logic [31:0] b);
    if ($signed(a) < $signed(b)) return CMP_LT;
    else if ($signed(a) > $signed(b)) return CMP_GT;
    else return CMP_EQ;
  endfunction
  ...
  // Golden ALU Model
  function automatic logic [31:0] golden_alu(input logic [31:0] src1, input logic [31:0] src2,
                                             input logic [4:0] shamt, input alu_op_t op);
    case (op)
