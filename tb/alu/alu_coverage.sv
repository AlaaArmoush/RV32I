...
class alu_coverage;
  ...
  function void sample (alu_item it);
    alu_cg.sample(it.alu_control, it.src1_class, it.src2_class, it.shamt_class, it.result_class,
                  it.zero, it.last_bit, logic'(it.alu_control inside {ALU_SLL, ALU_SRL, ALU_SRA}),
                  logic'(it.alu_control inside {ALU_SLT, ALU_SLTU}), classify_cmp(it.src1, it.src2
                  ));
  endfunction
